...
it <- filter(fcnz_relative,name=="it.val")
...
arranged_plot <- ggarrange(plot_fcnz_ec,plot_fcnz_ec,plot_fcnz_ec,legend2plot,ncol=4,heights = c(1,1,1,0.5),widths = c(1,1,1,0.5))+
  theme(plot.margin = margin(t = 0, r = 0, b = 0, l = 0,"cm"))

# Add the title
final_ec <- annotate_figure(arranged_plot , top = text_grob("Ecorregions vulnerabilty",color = "black", face = "bold", size = 12))

ggsave(plot = final_ec,filename = "/dados/pessoal/francisco/Linha_2_TradeHub/figures/outcome_ec_example_fig.png",height = 18,width = 32,units = "cm",dpi=100)
